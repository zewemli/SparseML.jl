...
function parse_commandline()
  ...
  @add_arg_table s begin
    ...
    arg_type = String
    "--logMap"
    help = "Map real values to their logs (ln)"
    action = :store_true
    "--train", "-t"
    help = "Training files"
    nargs = '*'
...
function main()
  ...
  canLabel = false
  model = nothing
  verbose = get(flags, "verbose", false)
  modelName = get(flags, "model", "naivebayes")
  logMap = get(flags, "logMap", false)
  ...
  if length( trainingFiles ) > 0
    ...
    start = time()
    model = train(model, params, Data.Dataset(trainingFiles, logMap, verbose))
    delta = time() - start
