...
type NBPredModel
    ...
    function NBPredModel(model::Model.Probability, ignorePriors::Bool)
        ...
        if !model.settings.ignorePriors
            base = .+(base, map(log, model.pclass))
        end
    
        return new(pMin, base, deltas, zeros( nclasses ))
    end
...
function train(model::NB, data::Data.Dataset)
    for row in Data.eachrow(data)
        Model.countRow(model.counts, row)    
    end
...
function label(model::NB, params::Dict, stream::Task)
    ...
    for row in stream
        produce( label(predModel, row, produceRanks) )
    end
